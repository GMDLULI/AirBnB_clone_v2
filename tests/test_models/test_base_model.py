...
class test_basemodel(unittest.TestCase):
    ...
    def test_todict(self):
        """testing the todict method of the model """
        i = self.value()
        n = i.to_dict()
        self.assertEqual(i.to_dict(), n)
        # Tests if it's a dictionary
        self.assertIsInstance(self.value().to_dict(), dict)
        # TEst if to_dict comtains accurate keys
        self.assertIn('id', self.value().todict())
        self.assertIn('created_at', self.value().to_dict())
        self.assertIn('updated_at', self.value().to_dict())
        # Tests if todict contains added attributes
        mdl = self.value()
        mdl.firstname = "Gugulethu"
        mdl.lastname = "Nhamuave"
        self.assertIn('firstname', mdl.to_dict())
        self.assertIn('lastname', mdl.to_dict())
        self.assertIn('firstname', self.value(firstname='Gugulethu').to_dict())
        self.assertIn('lastname', self.value(lastname='Nhamuave').to_dict())
        ...
        if os.getenv('HBNB_TYPE_STORAGE') != 'db':
            self.assertDictEqual(
                ...
                {
                    '__clas__': mdl.__class__.__name__,
                    'id': 'u-b34',
                    'age': 13
                 }
              )
